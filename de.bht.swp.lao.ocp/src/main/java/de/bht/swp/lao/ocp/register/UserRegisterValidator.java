...
import de.bht.swp.lao.ocp.auth.EmailValidator;
...
public class UserRegisterValidator implements Validator {
    ...
    public void validate(Object obj, Errors registerError) {
        ...
        if (registerFormData.getEmail() == null
                || registerFormData.getEmail().equals("")) {
            registerError.rejectValue("email", "email.field.required",
                    "Email is required!");
        } else {
            if (!isValidEmailAddress(registerFormData.getEmail())) {
                registerError.rejectValue("email", "email.invalid",
                        "Email isn't valid!");
            } else {
                if (userDao.findByEmail(registerFormData.getEmail()) != null) {
                    registerError.rejectValue("email", "email.already.used",
                            "Email is already in use");
                }
            }
        }

        if (registerFormData.getPassword() == null
                || registerFormData.getPassword().equals("")) {
            registerError.rejectValue("password", "password.field.required",
                    "Password is required!");
        } else {
            if (registerFormData.getPassword().length() < MIN_PASSWORD_LENGTH) {
                registerError.rejectValue("password", "password.field.length",
                        "Password needs to have at least "
                                + MIN_PASSWORD_LENGTH + " chars!");
            } else if (!registerFormData.getPassword().equals(
                    registerFormData.getPasswordvalidate())) {
                registerError.rejectValue("passwordvalidate",
